...
def setup_vscode(paths: ToolPaths) -> None:
    ...
    if not target.exists():
        target.write_text(json.dumps(src_cfg, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def setup_ai_agent(paths: ToolPaths) -> None:
    ...
    idx = ui.interactive_select("Setup AI Agent - Choose an option", choices, default_index=0)
...
def setup_claude_code(paths: ToolPaths) -> None:
    ...
    for dname in dir_items:
        ...
        if src_dir.is_dir():
            ...
            print(f"{ui.YELLOW}SKIP{ui.RESET} Missing directory in resources: {src_dir}")
...
def overwrite_agents_md(paths: ToolPaths, provider: str) -> None:
    ...
    try:
        ...
        print(f"{ui.GREEN}OVERWROTE{ui.RESET} AGENTS.md using template for {provider}: {dst}")
...
def verify(paths: ToolPaths) -> None:
    ...
    if tool_dir.name == ".x100":
        print(f"{ui.GREEN}OK{ui.RESET}    Current directory name is '.x100' -> {tool_dir}")
    else:
        all_ok = False
        print(
    ...
    if dotgit.is_dir() or dotgit.is_file():
        print(f"{ui.GREEN}OK{ui.RESET}    Outer directory appears to be a git repo -> {outer_dir}")
    else:
        all_ok = False
        print(f"{ui.RED}FAIL{ui.RESET}  Outer directory is not a git repo -> {outer_dir}")
    ...
    if all_ok:
        ...
        print(f"{ui.YELLOW}Some checks failed. Please review the output above.{ui.RESET}")
    ui.pause()
...
def init_project(paths: ToolPaths, wait_for_key: bool = True) -> None:
    ...
    if dotgit.exists():
        ...
        if ui.prompt_yes_no("Initialize git repository here?", default=True):
            try:
                result = subprocess.run(["git", "init", str(outer_dir)], capture_output=True, text=True, check=False)
                if result.returncode == 0:
                    print(f"{ui.GREEN}INIT{ui.RESET}     Git repository initialized in {outer_dir}")
                else:
                    print(f"{ui.RED}ERROR{ui.RESET}    git init failed: {result.stderr.strip()}")
    ...
    if cfg_path.exists():
        try:
            ...
            print(f"{ui.YELLOW}Warning:{ui.RESET} could not read existing config: {exc}")
    ...
    if "backend" not in config:
        ...
        be_idx = ui.prompt_choice("Backend language/framework:", backend_opts, default_index=0)
        config["backend"] = [None, "python", "django"][be_idx]

    if "frontend" not in config:
        frontend_opts = ["None", "NodeJS - NextJS"]
        fe_idx = ui.prompt_choice("Frontend language/framework:", frontend_opts, default_index=0)
    ...
    try:
        ...
        if link_path.is_symlink():
            if link_path.resolve() == cli_src.resolve():
                print(f"{ui.GREEN}OK{ui.RESET}      Symlink already exists -> {link_path}")
                need_link = False
            else:
                if ui.prompt_yes_no("A different symlink named 'x100' exists. Replace?", default=False):
                    link_path.unlink()
                else:
                    need_link = False
        elif link_path.exists():
            if ui.prompt_yes_no("An 'x100' file exists. Replace with symlink?", default=False):
                link_path.unlink()
            else:
                need_link = False
...
"""
                ...
                print(f"{ui.YELLOW}WRAPPED{ui.RESET}  Created shell wrapper at {link_path}")
    except Exception as exc:
    ...
    if sys.platform.startswith("win"):
        ...
        if not cmd_path.exists():
            try:
                ...
                print(f"{ui.YELLOW}Note:{ui.RESET} Could not create Windows launcher: {exc}")
...
def run_contribute(paths: ToolPaths) -> None:
    ...
    try:
        ...
        print(f"{ui.RED}ERROR{ui.RESET} 'bash' not found on PATH. Install bash to run contribute.")
        sys.exit(127)
...
def list_available_commands(paths: ToolPaths) -> None:
    ...
    if not available_dir.exists():
        print(f"{ui.RED}ERROR{ui.RESET} Available commands directory not found: {available_dir}")
    ...
    for filename in available_files:
        name = filename[:-3]
        status = f"{ui.GREEN}[ACTIVE]{ui.RESET}" if filename in active_files else f"{ui.DIM}[available]{ui.RESET}"
        description = ""
...
def list_available_agents(paths: ToolPaths) -> None:
    ...
    if not available_dir.exists():
        print(f"{ui.RED}ERROR{ui.RESET} Available agents directory not found: {available_dir}")
        ui.pause("Press Enter to continue…")
        return
                ...
                if end_idx > 0:
                    ...
                    for line in frontmatter.split('\n'):
                        if line.startswith('name:'):
                            ...
                            description = desc_line[:80] + "..." if len(desc_line) > 80 else (
                                desc_line.split('.')[0] + '.' if '.' in desc_line else desc_line
                            )
        except Exception:
            pass
...
def enable_command(paths: ToolPaths, command_name: str | None = None) -> None:
    ...
    if not command_name:
        command_name = _select_markdown(paths, available_dir, "Select command to enable:")
    if not command_name:
        return
